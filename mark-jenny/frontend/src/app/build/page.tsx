...
import { CheckCircle2, Code2, ExternalLink, FileCode2, Globe2, Loader2, Package, Play, Rocket, RotateCcw, SquareTerminal, Smartphone, Monitor, XCircle } from "lucide-react";
...
export default function BuildPage() {
  ...
  const [sandboxMode, setSandboxMode] = useState<"web" | "docker">("web");
  ...
  useEffect(() => { selfBuildApi.sandboxStatus().then((status) => { setDockerAvailable(status.docker.available); if (status.recommendation === "docker") setSandboxMode("docker"); }).catch(() => undefined); }, []);
  ...
  const start = async () => {
    ...
    try {
      const result = await selfBuildApi.start(prompt.trim(), sandboxMode, target);
  ...
  return <ProtectedLayout><div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex"><Sidebar isOpen={sidebarOpen} onToggle={() => setSidebarOpen(!sidebarOpen)} /><div className={`flex-1 min-w-0 flex flex-col transition-all ${sidebarOpen ? "ml-64" : "ml-16"}`}><Header /><main className="flex-1 overflow-auto p-4 lg:p-6"><div className="mx-auto max-w-[1500px] space-y-4">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Code2 className="h-5 w-5 text-blue-500" /><h1 className="text-2xl font-semibold">Build workspace</h1><Badge variant="outline">live</Badge></div><p className="mt-1 text-sm text-zinc-500">Build, inspect, preview, package, deploy, and push from one autonomous workspace.</p></div><Button onClick={start} disabled={starting || !prompt.trim()} className="bg-blue-600 hover:bg-blue-700"><Play className="mr-2 h-4 w-4" />{starting ? "Starting…" : "Start build"}</Button></div>
    <div className="grid gap-4 xl:grid-cols-[minmax(320px,0.8fr)_minmax(500px,1.2fr)]">
      <Card className="overflow-hidden"><CardHeader className="border-b pb-3"><CardTitle className="text-sm">Build brief</CardTitle></CardHeader><CardContent className="space-y-4 p-4"><Textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder="Build a property maintenance CRM with authentication, dashboard, mobile layout, and deployment configuration…" className="min-h-32 resize-y" /><div><p className="mb-2 text-xs font-medium text-zinc-500">Output target</p><div className="grid gap-2">{buildTargets.map((item) => { const Icon = targetIcons[item.id as keyof typeof targetIcons]; return <button key={item.id} onClick={() => setTarget(item.id)} className={`flex items-start gap-3 rounded-lg border p-3 text-left transition-colors ${target === item.id ? "border-blue-500 bg-blue-500/5" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}`}><Icon className="mt-0.5 h-4 w-4 text-blue-500" /><span><span className="block text-sm font-medium">{item.label}</span><span className="block text-xs text-zinc-500">{item.detail}</span></span>{target === item.id && <CheckCircle2 className="ml-auto h-4 w-4 text-blue-500" />}</button>; })}</div></div><div><p className="mb-2 text-xs font-medium text-zinc-500">Execution sandbox</p><div className="grid gap-2 sm:grid-cols-2"><button onClick={() => setSandboxMode("web")} className={`rounded-lg border p-3 text-left ${sandboxMode === "web" ? "border-blue-500 bg-blue-500/5" : "hover:bg-zinc-100 dark:hover:bg-zinc-800"}`}><span className="block text-sm font-medium">Web sandbox</span><span className="block text-xs text-zinc-500">Always available; isolated workspace and live artifacts.</span></button><button onClick={() => dockerAvailable && setSandboxMode("docker")} disabled={!dockerAvailable} className={`rounded-lg border p-3 text-left ${sandboxMode === "docker" ? "border-blue-500 bg-blue-500/5" : "hover:bg-zinc-100 dark:hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-50"}`}><span className="block text-sm font-medium">Docker sandbox</span><span className="block text-xs text-zinc-500">{dockerAvailable ? "Container-backed build execution." : "Docker runtime unavailable on this backend."}</span></button></div></div><div className="rounded-lg border bg-zinc-50 p-3 text-xs dark:bg-zinc-900"><p className="font-medium">{activeTarget.label} capability</p><p className="mt-1 text-zinc-500">Mark generates source and configuration, runs checks, records artifacts, and leaves the project ready for platform-specific signing or store credentials.</p></div></CardContent></Card>
