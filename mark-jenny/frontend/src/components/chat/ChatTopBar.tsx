...
import { SearchIcon, Settings, Moon, Sun, PanelRightClose, PanelRight, X, Plus, Pin, PinOff, MoreHorizontal, Pencil, Trash2, Globe } from "lucide-react";
          ...
          <>
            {browserSessions?.map((session) => (
              <div
                ...
                className={cn(
                  "flex items-center gap-1.5 rounded-md px-2 py-1.5 text-xs font-medium cursor-pointer transition-colors shrink-0 max-w-[180px]",
              ...
              >
                ...
                <span className="truncate">{session.name || session.url || `Session ${session.id.slice(0, 8)}`}</span>
              </div>
            ))}
            {(!browserSessions || browserSessions.length === 0) && (
              <span className="text-xs text-zinc-400 px-2">No active browser sessions</span>
            )}
          </>
        ) : (
          /* Chat tabs */
