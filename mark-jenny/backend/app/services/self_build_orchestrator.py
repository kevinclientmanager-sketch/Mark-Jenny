...
class SelfBuildOrchestrator:
    ...
    async def _modify_code(self, content: str, step: BuildStep, user_request: str) -> str:
        """Ask the model to extend the existing code with this feature.

        Previously this appended a hardcoded `def new_feature(): return {...}`
        stub, which is not an implementation of anything.
        """
        prompt = (
            f"Extend the following code to implement this change.\n\n"
            f"REQUEST: {user_request}\n"
            f"CHANGE: {step.description}\n\n"
            "Return the COMPLETE updated file, not a diff and not a fragment. "
            "Preserve existing behaviour and style.\n\n"
            f"--- BEGIN FILE ---\n{content[:20000]}\n--- END FILE ---"
        )
        code = _call_ai(prompt)
        if code and len(code) > 40 and "model unavailable" not in code.lower():
            return code
        # No model: refuse to pretend, and make the gap explicit in the file.
        return content + (
            f"\n\n# === Mark-Imti could not implement: {step.description} ===\n"
            f"# Request: {user_request[:200]}\n"
            "# Reason: no AI model was reachable, so no code was generated.\n"
            "# Connect a provider in Settings > AI Studio and re-run this build.\n"
        )

    async def _generate_skill(self, step: BuildStep, user_request: str) -> str:
        """Mark generates a new AI skill definition."""
        prompt = (
            "Write a Mark-Imti skill definition in Markdown for the following request.\n"
            f"REQUEST: {user_request}\n"
            f"SKILL: {step.description}\n\n"
            "Use these exact sections: Overview, When To Use, Instructions, Tools, "
            "Trigger Words, Quality Standards, Failure Handling. Be specific and actionable."
        )
        body = _call_ai(prompt)
        ...
        return (
            ...
            f"## Overview\n{user_request[:500]}\n\n"
            "## Instructions\n"
            "_Not generated: no AI model was reachable, so this skill has no real steps yet. "
            "Connect a provider in Settings > AI Studio and rebuild to populate it._\n"
        )

    async def _generate_endpoint(self, step: BuildStep, user_request: str) -> str:
        """Mark generates a new FastAPI endpoint module."""
        prompt = (
            "Generate a complete, runnable FastAPI router module for this endpoint.\n"
            f"REQUEST: {user_request}\n"
            f"ENDPOINT: {step.description}\n\n"
            "Rules: include `from fastapi import APIRouter` and a module-level `router = APIRouter()`. "
            "Use Pydantic models for request bodies. Add real handler logic that calls the "
            "appropriate service, never a stub that just returns {'status': 'ok'}. "
            "Return only Python code, no markdown fence."
        )
        code = _call_ai(prompt)
        if code and "APIRouter" in code and len(code) > 60:
            return code
        # No model available: emit a file that clearly refuses to pretend.
        return (
            '"""Endpoint could not be generated."""\n'
            "# Mark-Imti could not generate this endpoint because no AI model was reachable.\n"
            f"# Requested: {step.description}\n"
            "# Connect a provider in Settings > AI Studio and re-run the build.\n"
        )
